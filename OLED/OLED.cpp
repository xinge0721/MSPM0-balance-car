...
/**
  ...
  * @retval 无
  */
void OLED::I2C::Start(void)
{
	OLED* oled = (OLED*)((char*)this - offsetof(OLED, _i2c));
	oled->Send_SDA(1);
	oled->Send_SCL(1);
	oled->Send_SDA(0);
	oled->Send_SCL(0);
...
/**
  ...
  * @retval 无
  */
void OLED::I2C::SendByte(uint8_t Byte)
{
	OLED* oled = (OLED*)((char*)this - offsetof(OLED, _i2c));
	uint8_t i;
	for (i = 0; i < 8; i++)
	{
		oled->Send_SDA(Byte & (0x80 >> i));
		oled->Send_SCL(1);
		oled->Send_SCL(0);
	}
	oled->Send_SCL(1);	//额外的一个时钟，不处理应答信号
	oled->Send_SCL(0);
}
...
{
	_i2c.Start();
	_i2c.SendByte(0x78);		//从机地址
	_i2c.SendByte(0x00);		//写命令
	_i2c.SendByte(Command); 
	_i2c.Stop();
}
...
void OLED::Init(void)
{
	...
	OLED::WriteCommand(0xAE);	//关闭显示
	
	OLED::WriteCommand(0xD5);	//设置显示时钟分频比/振荡器频率
	OLED::WriteCommand(0x80);
	
	OLED::WriteCommand(0xA8);	//设置多路复用率
	OLED::WriteCommand(0x3F);
	
	OLED::WriteCommand(0xD3);	//设置显示偏移
	OLED::WriteCommand(0x00);
	
	OLED::WriteCommand(0x40);	//设置显示开始行
	
	OLED::WriteCommand(0xA1);	//设置左右方向，0xA1正常 0xA0左右反置
	
	OLED::WriteCommand(0xC8);	//设置上下方向，0xC8正常 0xC0上下反置

	OLED::WriteCommand(0xDA);	//设置COM引脚硬件配置
	OLED::WriteCommand(0x12);
	
	OLED::WriteCommand(0x81);	//设置对比度控制
	OLED::WriteCommand(0xCF);

	OLED::WriteCommand(0xD9);	//设置预充电周期
	OLED::WriteCommand(0xF1);

	OLED::WriteCommand(0xDB);	//设置VCOMH取消选择级别
	OLED::WriteCommand(0x30);

	OLED::WriteCommand(0xA4);	//设置整个显示打开/关闭

	OLED::WriteCommand(0xA6);	//设置正常/倒转显示

	OLED::WriteCommand(0x8D);	//设置充电泵
	OLED::WriteCommand(0x14);

	OLED::WriteCommand(0xAF);	//开启显示
