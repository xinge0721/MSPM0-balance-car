...
// 电机方向控制宏定�?
#define MOTOR_left_DIRECTION 1  // 1为正常方向，0为反�?
#define MOTOR_right_DIRECTION 0  // 1为正常方向，0为反�?
...
void Control_speed(int left_speed,int right_speed)
{
    if(left_speed>0)
    {
        Control_AIN1_ON();
        Control_AIN2_OFF();
        DL_TimerA_setCaptureCompareValue(PWM_INST,left_speed,DL_TIMER_CC_1_INDEX);
    ...
    if(right_speed>0)
    {
        Control_BIN1_ON();
        Control_BIN2_OFF();
        DL_TimerA_setCaptureCompareValue(PWM_INST,right_speed,DL_TIMER_CC_0_INDEX);
    }
